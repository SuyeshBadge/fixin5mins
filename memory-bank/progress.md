...
4. **Resolved Issues**:
   - ✅ **FIXED**: Cloudinary images not being deleted in scheduled posts (CRITICAL FIX APPLIED)
   - ✅ **FIXED**: Public ID extraction logic was broken, causing failed deletions
   - ✅ **FIXED**: Workflow mismatch between direct and scheduled posting cleanup
...
## [2024-06-01] Time Zone Configuration
- The setup.sh script now sets the system time zone to Asia/Kolkata (IST) for both Linux and macOS.
- It exports TZ=Asia/Kolkata for the current session.
- The .env template now includes TZ=Asia/Kolkata by default.
- This ensures consistent IST time zone handling for the app and scripts.

## [2024-06-XX] Fixed issue where emoji icons were not rendered in generated images on Google VM by installing the `fonts-noto-color-emoji` package. This is required for proper emoji rendering in headless Chrome/Puppeteer environments.
